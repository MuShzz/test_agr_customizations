...
CREATE VIEW [bc_rest_cus].[v_PURCHASE_ORDER_RECEIVED_HISTORY] AS
	SELECT 
		...
		CAST(prh.[LocationCode]  AS NVARCHAR(255))		AS [LOCATION_NO], -- Always empty in this tenant_225_stg?
	...
	FROM bc_rest.purch_rcpt_header prh	
			 INNER JOIN bc_rest.purch_rcpt_line prl ON prl.DocumentNo = prh.No
			 LEFT JOIN bc_rest.item_ledger_entry ile
					   ON ile.DocumentNo = prh.No AND ile.LocationCode = prh.LocationCode AND ile.ItemNo = prl.No
