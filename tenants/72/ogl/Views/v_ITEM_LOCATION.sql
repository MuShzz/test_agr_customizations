...
CREATE VIEW [ogl_cus].[v_ITEM_LOCATION] AS

     SELECT DISTINCT
          CAST(st.stcode AS NVARCHAR(255)) 											AS [ITEM_NO]
          ,CAST(st.depot AS NVARCHAR(255)) 											AS LOCATION_NO
          ,CAST(0 AS DECIMAL(18,4)) 												AS [REORDER_POINT]
          ,CAST(IIF(st.maxlev=0,NULL,st.maxlev) AS DECIMAL(18,4)) 					AS [MAX_STOCK]
          ,CAST(CASE WHEN s.obsoleteind = 'N' THEN 0 ELSE 1 END AS BIT)				AS [CLOSED]
		  ...
		  ,CAST(st.minlev AS DECIMAL(18,4))                                         AS [SAFETY_STOCK_UNITS]
          ,CAST(NULL AS DECIMAL(18,4))                                              AS [MIN_DISPLAY_STOCK]
		  ,CAST(NULL AS BIT)														AS [SPECIAL_ORDER]
     FROM [ogl].[STDepotStockLevels] st
     INNER JOIN [ogl].[STStockDetails] s ON s.stcode = st.stcode
	WHERE s.category <> 'XXX'
	    AND s.pgroup NOT IN ('12','994','995','996','997','998','999') -- removed group 1 from filter AL-17
