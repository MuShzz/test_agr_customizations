...
CREATE VIEW [cus].[v_ITEM_LOCATION] AS
	...
	SELECT 
		CAST(sku.[ItemNo] + CASE WHEN sku.[VariantCode] = '' THEN '' ELSE '-' + sku.[VariantCode] END AS NVARCHAR(255)) AS [ITEM_NO]
		...
		,CAST(sku.Company AS NVARCHAR(255))																AS [company]
    FROM cus.stock_keeping_unit sku
