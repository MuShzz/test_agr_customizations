...
(
    ...
    FROM fo_cus.ProductOrderSettings AS pos
)
			SELECT DISTINCT
	   ...
	   LEFT   JOIN pos_one                   AS pos
           ON  pos.ItemNumber = p.ItemNumber
           AND pos.rn = 1     
	   LEFT JOIN fo_cus.ProductInfo pi ON p.ITEMNUMBER = pi.PRODUCTNUMBER
	   LEFT JOIN fo_cus.ProductExtraInfo pei ON pei.ITEMNUMBER = p.ITEMNUMBER
	   LEFT JOIN fo.StockLevel sl ON sl.ITEMNUMBER = p.ITEMNUMBER
	   WHERE pi.PRODUCTNAME IS NOT NULL AND pei.PRODUCTCOVERAGEGROUPID NOT LIKE '%MTO%'
