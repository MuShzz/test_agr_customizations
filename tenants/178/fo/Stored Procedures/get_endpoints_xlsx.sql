CREATE PROCEDURE [fo_cus].[get_endpoints_xlsx]
(
...
BEGIN
	...
	-- Populating the #endpoints temporary table with data from [dbo].[endpoints], 
	...
	-- Declare variables
	DECLARE @id INT, @preSqlExec NVARCHAR(MAX), @pre_sql NVARCHAR(MAX), @sql NVARCHAR(MAX), @params NVARCHAR(MAX), @output NVARCHAR(MAX), @pre_sql_value NVARCHAR(MAX)
  ...
  SELECT 
	  ...
	  'SELECT ' + COALESCE(e.select_sql,(SELECT STRING_AGG(QUOTENAME(c.[column_name]),',') 
	   ...
	   + ' FROM '+ COALESCE(e.from_sql, e.endpoint+'.'+e.endpoint_schema)
  + IIF(
      e.task_type = 7,
      ' WHERE "TransNum" > ' + @max_entry_no
        + IIF(e.where_sql IS NOT NULL, ' AND ' + e.where_sql, ''),
      IIF(e.where_sql IS NOT NULL, ' WHERE ' + e.where_sql, '')
    ) AS query,
