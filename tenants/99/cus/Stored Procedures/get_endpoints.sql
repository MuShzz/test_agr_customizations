...
INSERT INTO #endpoints ([id], [step_name], [endpoint_schema], [endpoint], [task_type], [db_schema], [db_table], [priority], [is_active], [pre_sql_exec], [pre_sql], [select_sql], [from_sql], [where_sql], [post_sql])
SELECT [id], [step_name], [endpoint_schema], [endpoint], [task_type], [db_schema], [db_table], [priority], [is_active], [pre_sql_exec], [pre_sql], [select_sql], [from_sql], [where_sql], [post_sql]
FROM [cus].[endpoints];
  ...
  SELECT 
	  ...
	  'SELECT ' + COALESCE(e.select_sql,(SELECT STRING_AGG(QUOTENAME(c.[column_name]),',') 
	   ...
	   + ' FROM '+COALESCE(e.from_sql,QUOTENAME(ISNULL(e.endpoint_schema,'dbo'))+'.'+QUOTENAME(IIF(@CompanyName IS NOT NULL AND @CompanyName <> '',@CompanyName+'$','')+e.[endpoint]+IIF(@TablePostfix IS NOT NULL AND @TablePostfix <> '','$'+@TablePostfix,''))) 
