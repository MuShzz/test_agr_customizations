...
CREATE VIEW [bc_sql_cus].[v_ITEM_LOCATION] AS
    ...
    SELECT DISTINCT
        --CAST(sku.[Item No_] +
        --        IIF(iv.[Code] IS NULL OR iv.[Code] = '', '', '-' + iv.[Code]) AS NVARCHAR(255)) AS [ITEM_NO]
        ...
        ,CAST(sku.[Reorder Point] AS DECIMAL(18,4)) AS [MIN_DISPLAY_STOCK]
		...
		,CAST(CASE WHEN it.[Blocked] = 1 THEN 1
				  ...
				  WHEN it.[Vendor No_] IN ('5000', '2600') AND it.[Purchasing Blocked] = 1 THEN 1
				  WHEN it.[Vendor No_] = '5000' AND ix.[Vendor Stock$63a5512e-ef5c-4cc4-ac67-fc1739ce27d8] = '' THEN 1
