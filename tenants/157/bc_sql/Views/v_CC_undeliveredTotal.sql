...
AS
	...
	WITH CTE_UPO AS (
		...
		WHERE
			upo.PURCHASE_ORDER_NO NOT LIKE 'STOCK%' --Added in because stock on G-locations should not be counted in total undelivered.
	...
	CTE_UTO AS (
		...
		WHERE 
			uto.ORDER_FROM_LOCATION_NO NOT LIKE 'G-%' --Added in because transfers from G- locations should not be in total undelivered, stock on G- locations is default undelivered.
