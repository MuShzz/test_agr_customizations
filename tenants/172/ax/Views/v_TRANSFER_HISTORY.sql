...
AS
	...
	FROM (
			...
			FROM 
				[ax_cus].INVENTTRANS it 
				INNER JOIN [ax_cus].INVENTTRANSORIGIN ito ON it.INVENTTRANSORIGIN = ito.RECID AND  it.PARTITION = ito.PARTITION AND it.DATAAREAID = ito.DATAAREAID
				...
				INNER JOIN ax_cus.INVENTLOCATION loc ON loc.INVENTLOCATIONID = id.INVENTLOCATIONID AND loc.DATAAREAID = id.DATAAREAID AND loc.PARTITION = id.PARTITION
			WHERE 
