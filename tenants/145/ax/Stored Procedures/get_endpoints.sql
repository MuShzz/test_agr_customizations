CREATE PROCEDURE [ax_cus].[get_endpoints]
(
...
BEGIN
    ...
    DECLARE @id INT, @preSqlExec NVARCHAR(MAX), @pre_sql NVARCHAR(MAX), @sql NVARCHAR(MAX), @params NVARCHAR(MAX), @output NVARCHAR(MAX), @max_entry_no NVARCHAR(MAX), @max_entry_buffer NVARCHAR(MAX)
    
    SET @max_entry_no = CAST(CAST((SELECT dateadd(day,-5,(SELECT CAST(MAX([DATEPHYSICAL]) as date))) FROM ax.INVENTTRANS where DATEPHYSICAL <= getdate()) AS DATE) as NVARCHAR);
    -- Declare cursor
  ...
  group by [ITEMID],
    ...
    SELECT
        ...
        + CASE
              WHEN e.[db_table] = 'inventtransorigin' THEN ' WHERE ito.dataareaid IN (''hag'')'
              WHEN e.[db_table] in ('INVENTITEMGROUPITEM','INVENTMODELGROUPITEM') THEN ' WHERE itemdataareaid IN (''hag'')'
              WHEN EXISTS (
                  SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = e.db_schema AND TABLE_NAME = e.db_table AND COLUMN_NAME  = 'dataareaid'
              ) THEN ' WHERE dataareaid IN (''hag'')'
              ELSE ''
              END
        ...
        CASE WHEN e.task_type = 0 THEN 'TRUNCATE TABLE '+QUOTENAME(e.[db_schema])+'.'+QUOTENAME(e.[db_table])
             WHEN e.db_table = 'INVENTTRANS' THEN  'delete from ax.INVENTTRANS where (DATEPHYSICAL >= '''+@max_entry_no+''' or DATEPHYSICAL = ''1900-01-01'')'
             WHEN e.db_table = 'INVENTTRANSORIGIN' THEN  'DELETE O FROM ax.INVENTTRANSORIGIN O INNER JOIN ax.INVENTTRANS T ON O.RECID = T.INVENTTRANSORIGIN WHERE (T.DATEPHYSICAL >= '''+@max_entry_no+''' or DATEPHYSICAL = ''1900-01-01'')'
