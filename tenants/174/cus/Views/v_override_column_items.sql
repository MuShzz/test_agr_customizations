...
AS
	SELECT CAST(i.ITEMID AS NVARCHAR(255))  AS [no]
		 ...
		 , NULL                         AS [specialOrder] --IIF(NULL=1, 'true', 'false') AS [specialOrder]
		 , CAST(IIF(i.REQGROUPID = 'Transit',1,0) AS BIT)  AS [excludeFromAgr] --IIF(NULL=1, 'true', 'false') AS [excludeFromAgr]
	 FROM cus.INVENTTABLE i
