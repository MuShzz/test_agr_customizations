...
CREATE VIEW [bc_sql_cus].[v_ITEM] AS
    ...
    FROM
		...
		LEFT JOIN bc_sql.ItemUnitOfMeasure kassi	ON kassi.[Item No_] = i.[No_] AND kassi.Code = 'BOX' AND kassi.[Qty_ per Unit of Measure] > 0
		LEFT JOIN bc_sql.ItemUnitOfMeasure stk		ON stk.[Item No_] = i.[No_] AND stk.Code = 'PCE' AND kassi.[Qty_ per Unit of Measure] > 0
    WHERE
