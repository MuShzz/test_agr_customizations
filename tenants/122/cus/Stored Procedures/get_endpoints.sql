...
INSERT INTO #endpoints ([id], [step_name], [endpoint], [endpoint_schema], [task_type], [db_schema], [db_table], [priority], [is_active], [pre_sql_exec], [pre_sql], [select_sql], [from_sql], [where_sql], [post_sql])
SELECT [id], [step_name], [endpoint], [endpoint_schema], [task_type], [db_schema], [db_table], [priority], [is_active], [pre_sql_exec], [pre_sql], [select_sql], [from_sql], [where_sql], [post_sql]
FROM [cus].[endpoints];
	...
	SET @max_entry_no = (SELECT CAST(ISNULL(MAX([TransNum]),0) AS NVARCHAR(255)) FROM cus.OINM)
	
  SELECT 
        ISNULL(e.[endpoint_schema],'dbo') AS TableSchema_S
	  ...
	  'SELECT ' + COALESCE(e.select_sql,(SELECT STRING_AGG(('"'+ c.column_name + '"'),',') 
                        FROM [dbo].[v_db_object_info] AS c
                               WHERE  c.[schema_name] = e.[db_schema]
                                  AND c.[object_name] = e.[db_table]))
	   + ' FROM '+COALESCE(e.from_sql,ISNULL(e.endpoint_schema + '.','dbo.') +(IIF(@CompanyName IS NOT NULL AND @CompanyName <> '',@CompanyName+'$'+e.[endpoint],e.endpoint)+IIF(@TablePostfix IS NOT NULL AND @TablePostfix <> '','$'+@TablePostfix,''))) 
	   + IIF(e.task_type = 7, ' WHERE "TransNum" >'+@max_entry_no + IIF(e.where_sql IS NOT NULL,' AND '+e.where_sql,''), IIF(e.where_sql IS NOT NULL,' WHERE '+e.where_sql,'')) AS query,
