...
-- ===============================================================================
create PROCEDURE [nav_cus].[get_endpoints]
(
...
BEGIN
	...
	-- Populating the #endpoints temporary table with data from [nav].[endpoints], 
...
FROM [nav].[endpoints];
	...
	IF OBJECT_ID('nav_cus.endpoints', 'U') IS NOT NULL
	BEGIN
		...
		EXEC('
			...
			FROM #endpoints e
			INNER JOIN nav_cus.endpoints ce ON e.[id] = ce.[id]
		');
  ...
  SELECT 
	  ...
	  'SELECT ' + COALESCE(e.select_sql,(SELECT STRING_AGG(IIF(data_type = 'Decimal','cast('+QUOTENAME(c.[column_name])+' as decimal(28,10)) as '+QUOTENAME(c.[column_name]), QUOTENAME(c.[column_name])),',')
                        FROM [dbo].[v_db_object_info] AS c
                               WHERE  c.[schema_name] = e.[db_schema]
                                  AND c.[object_name] = e.[db_table]))
	   + ' FROM '+COALESCE(e.from_sql,'dbo.'+QUOTENAME(@CompanyName+'$'+e.[endpoint]+IIF(@TablePostfix IS NOT NULL AND @TablePostfix <> '','$'+@TablePostfix,'')))  
