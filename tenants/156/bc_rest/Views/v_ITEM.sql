...
AS
    ...
    SELECT
       ...
       -- CAST(CASE WHEN c.[ParentCategory] <> '' THEN i.[ItemCategoryCode] ELSE '' END AS NVARCHAR(255))						AS ITEM_GROUP_NO_LVL_2,
	    CAST(pg.[Description] AS NVARCHAR(255))																				AS ITEM_GROUP_NO_LVL_2,
    ...
    FROM
		...
		LEFT JOIN [bc_rest_cus].[ItemProductGroup] ipg	ON i.[No] = ipg.[No]
		LEFT JOIN [bc_rest_cus].[ProductGroup] pg		ON pg.[Code] = ipg.[LSCRetailProductCode]
