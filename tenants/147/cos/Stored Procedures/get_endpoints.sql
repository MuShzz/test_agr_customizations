...
FROM [cos].[endpoints];
	...
	BEGIN
		SELECT 
			...
			' FROM ' + QUOTENAME(e.endpoint_schema, '') + '.' + QUOTENAME(e.endpoint, '') +
		...
		SELECT 
			...
			CASE 
				...
				WHEN e.step_name = 'AGR_TRANSFER_HISTORY' AND EXISTS (SELECT 1 FROM cos.AGR_TRANSFER_HISTORY) THEN
					' WHERE transaction_id > ' + CAST(ISNULL((SELECT MAX(th.TRANSACTION_ID) FROM cos.AGR_TRANSFER_HISTORY th),0) AS NVARCHAR(MAX))
				ELSE 
					'' 
			END AS query,
			'' AS pre_copy_script,
	...
	BEGIN
		SELECT 
			...
			e.db_table AS TableName_T,
			'SELECT ' + COALESCE(e.select_sql,e.columns_to_merge, '') + 
			' FROM `' + COALESCE(e.endpoint_schema, '') + '`.`' + COALESCE(e.endpoint, '') + '`' AS query,
		...
		SELECT 
			...
			e.db_table AS TableName_T,
			'SELECT ' + COALESCE(e.select_sql,e.columns_to_merge, '') + 
			' FROM `' + COALESCE(e.endpoint_schema, '') + '`.`' + COALESCE(e.endpoint, '') + '`' +
			CASE 
				WHEN e.step_name = 'AGR_SALES_HISTORY' AND EXISTS (SELECT 1 FROM cos.AGR_SALES_HISTORY) THEN 
					' WHERE transaction_id > ' + CAST((SELECT MAX(sah.TRANSACTION_ID) FROM cos.AGR_SALES_HISTORY sah) AS NVARCHAR(MAX)) 
				WHEN e.step_name = 'AGR_STOCK_HISTORY' AND EXISTS (SELECT 1 FROM cos.AGR_STOCK_HISTORY) THEN  
