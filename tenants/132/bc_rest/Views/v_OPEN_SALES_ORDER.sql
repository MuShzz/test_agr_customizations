...
AS
	...
	SELECT
		CAST(poc.ProdOrderNo AS NVARCHAR(128)) AS SALES_ORDER_NO,
        CAST(poc.ItemNo + CASE WHEN ISNULL(poc.VariantCode, '') = '' THEN '' ELSE '-' + poc.VariantCode END AS NVARCHAR(255)) AS [ITEM_NO],
        CAST(poc.LocationCode AS NVARCHAR(255)) AS LOCATION_NO,
        SUM(CAST(poc.RemainingQtyBase AS DECIMAL(18,4))) AS QUANTITY,
		CAST('agr_no_customer' AS NVARCHAR(255))  AS CUSTOMER_NO,
        CAST(poc.DueDate AS DATE) AS DELIVERY_DATE
    FROM
        [bc_rest_cus].prod_order_component poc
    WHERE
        poc.Status IN ('Firm Planned','Released')
		AND poc.LocationCode IN ('PANEL-PROD', 'SSA-PROD', 'STORE', 'FGI','CABLECELL' )
    GROUP BY
        poc.ProdOrderNo,poc.ItemNo, poc.VariantCode, poc.LocationCode,poc.DueDate
    HAVING SUM(CAST(poc.RemainingQtyBase AS DECIMAL(18,4))) <> 0
