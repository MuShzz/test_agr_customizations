...
CREATE VIEW [sap_b1_cus].[v_ITEM] AS
	   SELECT
        ...
        CAST(COALESCE(cp.Price,0.0000) AS DECIMAL(18,4)) AS [SALE_PRICE],
        CAST(it.AvgPrice AS DECIMAL(18,4)) AS [COST_PRICE],
        ...
        CAST(IIF(it.QryGroup31 = 'Y' , 1, 0) AS BIT) AS [SPECIAL_ORDER],
        CAST(COALESCE(it.ReorderPnt,0) AS DECIMAL(18,4)) AS [REORDER_POINT],
		CAST(0 AS BIT) AS [INCLUDE_IN_AGR],
		CAST(IIF((it.[PrchseItem] ='Y' OR  it.[SellItem] ='Y') AND  it.[InvntItem] ='Y' AND  it.[frozenFor] = 'N', 0, 1) AS BIT) AS [CLOSED]   --- Para todos os sap_b1 com BOM
    FROM
        [sap_b1].OITM it
