...
FROM [ax].[endpoints] WHERE is_active = 1;
  ...
  SELECT 
	  ...
	  'SELECT ' 
       ...
       + 
	   CASE WHEN e.task_type IN (0,7) THEN
	...
	END
       + IIF(e.where_sql IS NOT NULL, 
			 ...
			 WHEN e.task_type = 2 THEN
			 +  ' WHERE ' + e.where_sql END
             
			 AS query,
       CASE WHEN e.task_type in (0,2) THEN 'TRUNCATE TABLE '+QUOTENAME(e.[db_schema])+'.'+QUOTENAME(e.[db_table])
