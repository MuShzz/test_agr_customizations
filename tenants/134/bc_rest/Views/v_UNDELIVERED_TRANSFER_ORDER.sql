...
-- Description: Transfer order mapping from bc rest to adi format
--
-- 26.09.2024.TO    Created
-- ===============================================================================
CREATE VIEW [bc_rest_cus].[v_UNDELIVERED_TRANSFER_ORDER]
AS
    ...
    FROM
        [bc_rest].transfer_line
    WHERE
        [OutstandingQtyBase] > 0 
    GROUP BY
        [DocumentNo], [TransferfromCode], [ItemNo], [TransfertoCode], CAST(IIF([ReceiptDate]='0001-01-01 00:00:00.000', GETDATE(),[ReceiptDate]) AS DATE)
