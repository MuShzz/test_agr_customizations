...
CREATE PROCEDURE [cus].[get_endpoints_COMMERCIAL]
(
...
SELECT [id], [step_name], [endpoint], [task_type], [db_schema], [db_table], [priority], [is_active], [pre_sql_exec], [pre_sql], [select_sql], [from_sql], [where_sql], [post_sql], 'COM' as [Company]
FROM [cus].[endpoints];
  ...
  FROM #endpoints e
  WHERE e.is_active = 1 AND e.[Company] = 'COM'
