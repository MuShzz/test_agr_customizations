...
AS
	...
	SELECT
        ...
        CAST([nav].[LeadTimeConvert] (vex.[Purchase Lead Time]) AS SMALLINT)                                        AS [ORDER_FREQUENCY_DAYS],
		...
		CAST(iif(i.[Vendor Item No_] LIKE 'öö%', 1, i.[Blocked]) AS BIT)                                            AS CLOSED,
    ...
    FROM
		...
		LEFT JOIN nav_cus.VendorExtraInfo vex ON i.[Vendor No_]=vex.No_
    WHERE
        i.[Type]  = 0
    AND 
