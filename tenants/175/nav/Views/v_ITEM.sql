...
--
-- 30.04.2025.GM	Created for custom item mapping from NAV
-- ===============================================================================

CREATE VIEW [nav_cus].[v_ITEM]
AS

	--This is used in the Plus setup
	WITH cte AS
	(
		SELECT
			CAST([Item No_] + CASE WHEN ISNULL([Variant Code],'') = '' THEN '' ELSE '-' + [Variant Code] END AS NVARCHAR(255)) 	AS ItemNo,
			CAST(SUM(Quantity) AS DECIMAL(18,4))																				AS UnitQty
		FROM
			nav_cus.PendingSalesOrderLine
		WHERE
			Released = 0
		GROUP BY
	...
	SELECT
		...
		CAST(CASE WHEN i.Blocked = 1 THEN 1
				  WHEN iei.[Deliv_ Performance Requirement] = 0 AND ISNULL(cte.UnitQty,0) = 0 THEN 1
				  WHEN iei.[Deliv_ Performance Requirement] = 0 and ISNULL(cte.UnitQty,0) <> 0  THEN 0
				  WHEN iei.[Item Status] NOT IN (0,1) AND ISNULL(cte.UnitQty,0) = 0 THEN 1
				  WHEN iei.[Item Status] NOT IN (0,1) AND ISNULL(cte.UnitQty,0) <> 0 THEN 0
        ...
        CAST(iei.[Emergency Stock] AS DECIMAL(18,4))                                                                AS [MIN_DISPLAY_STOCK],
    ...
    FROM
		...
		INNER JOIN nav_cus.ItemExtraInfo iei ON iei.No_ = i.No_
		INNER JOIN nav_cus.DimensionValue dv ON dv.Code = iei.[Global Dimension 1 Code]
		LEFT JOIN cte ON cte.ItemNo = i.No_
    ...
    WHERE
        i.[Type] = 0
		AND iei.[Item Status] IN (0,1,5)
		AND dv.[Valid in AGR] = 1
    AND 
