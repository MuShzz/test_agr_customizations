...
AS
SELECT
		 CAST(pei.ITEMNUMBER + CASE WHEN pei.[COVERAGEPRODUCTCONFIGURATIONID] IS NULL OR pei.[COVERAGEPRODUCTCONFIGURATIONID] = '' THEN '' ELSE '-' + ISNULL(pei.COVERAGEPRODUCTCONFIGURATIONID, '')END AS NVARCHAR(255)) AS [ITEM_NO]        
		,CAST(pei.COVERAGEWAREHOUSEID AS NVARCHAR(255)) AS [LOCATION_NO]
        ,CAST(CASE WHEN pei.COVERAGESITEID = 'EUROPE' AND pei.PRODUCTCOVERAGEGROUPID = 'MIN/MAX' THEN pei.MINIMUMONHANDINVENTORYQUANTITY ELSE NULL END AS DECIMAL(18,4)) AS [REORDER_POINT]
        ,CAST(pei.MINIMUMONHANDINVENTORYQUANTITY AS DECIMAL(18,4)) AS [SAFETY_STOCK_UNITS]
        ,CAST(NULL AS DECIMAL(18,4)) AS [MIN_DISPLAY_STOCK]
        ,CAST(CASE WHEN pei.COVERAGESITEID = 'EUROPE' AND pei.PRODUCTCOVERAGEGROUPID = 'MIN/MAX' THEN pei.MAXIMUMONHANDINVENTORYQUANTITY ELSE NULL END AS DECIMAL(18,4)) AS [MAX_STOCK]
		...
		LEFT JOIN fo.ProductInfo pi ON pei.ITEMNUMBER = pi.PRODUCTNUMBER
		LEFT JOIN fo.ProductOrderSettings pos ON pos.ITEMNUMBER = pei.ITEMNUMBER
		LEFT JOIN fo_cus.CustomColumns_Products cc ON cc.ITEMNUMBER = pei.ITEMNUMBER
