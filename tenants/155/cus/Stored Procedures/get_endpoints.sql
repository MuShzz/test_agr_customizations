...
BEGIN
	...
	(
		...
		[endpoint_schema] [NVARCHAR](100) NULL,
...
FROM [cus].[endpoints];
  ...
  SELECT 
	  ...
	  'SELECT ' + COALESCE(e.select_sql,(SELECT STRING_AGG(QUOTENAME(c.[column_name]),',') 
	   ...
	   + ' FROM '+COALESCE(e.from_sql,ISNULL(e.endpoint_schema,'dbo.')+QUOTENAME(@CompanyName+'$'+e.[endpoint]+IIF(@TablePostfix IS NOT NULL AND @TablePostfix <> '','$'+@TablePostfix,''))) 
