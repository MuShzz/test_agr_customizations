...
SELECT [id], [step_name], [endpoint], NULL AS [TablePostfix], [task_type], [db_schema], [db_table], [priority], [is_active], [pre_sql_exec], [pre_sql], [select_sql], [from_sql], [where_sql], [post_sql]
FROM [bc_sql].[endpoints];
  ...
  SELECT 
	  ...
	  'SELECT ' + COALESCE(e.select_sql,(SELECT STRING_AGG(IIF(data_type = 'Decimal','cast('+QUOTENAME(c.[column_name])+' as decimal(28,10)) as '+QUOTENAME(c.[column_name]), QUOTENAME(c.[column_name])),',') 
	   ...
	   + ' FROM '+COALESCE(e.from_sql,'dbo.'+QUOTENAME(@CompanyName+'$'+e.[endpoint]+IIF(e.[TablePostfix] IS NOT NULL,'$'+e.[TablePostfix],'$'+@TablePostfix)))
