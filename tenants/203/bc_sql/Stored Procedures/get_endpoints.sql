...
BEGIN
    ...
    (
        ...
        [TablePostfix] [NVARCHAR](MAX) NULL
    );

    INSERT INTO #endpoints ([id], [step_name], [endpoint], [task_type], [db_schema], [db_table], [priority],
    ...
    IF OBJECT_ID('bc_sql_cus.endpoints', 'U') IS NOT NULL
		...
		');
            ...
            -- Insert new rows
            EXEC ('
			INSERT INTO #endpoints ([id], [step_name], [endpoint], [task_type], [db_schema], [db_table], [priority], [is_active], [pre_sql_exec], [pre_sql], [select_sql], [from_sql], [where_sql], [post_sql],[TablePostfix])
			SELECT [id], [step_name], [endpoint], [task_type], [db_schema], [db_table], [priority], [is_active], [pre_sql_exec], [pre_sql], [select_sql], [from_sql], [where_sql], [post_sql],[TablePostfix]
    ...
    SET @max_sql = '
        ...
        LEFT JOIN bc_sql.companies cp ON ile.COMPANY = cp.company and cp.is_active = 1
    ...
    SELECT 'dbo'                                                         AS TableSchema_S
        ...
        + ' FROM ' +
           COALESCE(e.from_sql, 'dbo.' + QUOTENAME(ISNULL(cp.table_pre_fix, @CompanyName) + '$' + e.[endpoint] +
                                                   IIF(COALESCE(cp.table_post_fix, e.TablePostfix, @TablePostfix) IS NOT NULL AND
                                                       COALESCE(cp.table_post_fix, e.TablePostfix, @TablePostfix) <> '',
                                                       '$' + COALESCE(cp.table_post_fix, e.TablePostfix, @TablePostfix), '')))
        + IIF(e.task_type = 7,
    ...
    FROM #endpoints e
             left join bc_sql.companies cp on cp.is_active = 1
