...
AS
       ...
       SELECT
            ...
            CAST(case when id.INVENTLOCATIONID = 'TOLL' then 'ORA' else id.INVENTLOCATIONID END AS NVARCHAR(255))				AS [LOCATION_NO],
	...
	WHERE	(pt.PurchStatus = 0 OR pt.PurchStatus = 1) and id.dataareaid <> 'mfk' and p.RemainPurchPhysical > 0 
				--and st.CUSTACCOUNT is null
				and pt.StatProcID <> 'Gámur' --and i.id in (3140,3128,3127,3086,3084,3083,3079)
				and it.STATUSRECEIPT = 5
				GROUP BY pt.PURCHID,it.ITEMID, case when id.INVENTLOCATIONID = 'TOLL' then 'ORA' else id.INVENTLOCATIONID END,CAST(pt.DELIVERYDATE AS DATE),pt.PURCHSTATUS, p.DATAAREAID
